---
name: tdd
description: Use when writing any implementation code — any new function, class, endpoint, component, or feature. Enforces the RED-GREEN-REFACTOR cycle: write the failing test first, watch it fail for the right reason, then write the minimal code to pass, then verify, then refactor. Also use for bug fixes: write a test that reproduces the bug before writing the fix.
---
