---
description: Write a comprehensive implementation plan from a spec or requirements. Maps files, decomposes into TDD tasks with exact code and commands, and self-reviews for completeness before handoff to /task or /sprint.
allowed-tools: Glob, Grep, Read, LS, Write, Bash(git log:*), TodoWrite
---
...
- Split by responsibility, not by technical layer (not "controllers/", "models/", "utils/")
...
3. **Consistency** — do type names, function names, and method signatures match across all tasks? A function called `clearLayers()` in Task 3 but `clearFullLayers()` in Task 7 is a bug.
...
1. /task — autonomous delegation (tech-lead → engineers → QA → security → review)
2. Execute inline — I'll work through tasks in this session
